<div
  ...
  x-data='searchCemetery(@json($params))'
>
  <template x-if="cemeterySearchErrors">
    <div class="text-red-600 flex w-full" x-html="cemeterySearchErrors?.message ?? ''"></div>
  </template>
  <form
    {{ $attributes->merge([
      'class' => 'flex gap-4'
  ...
  >
    ...
    <div>
      <x-shared.btn
        ...
        x-bind:disabled="isCemeterySearchLoading"
      >
        {{ $actionText }}
      </x-shared.btn>
    </div>
